...
application is provider-neutral and complete for dry-run operation. An SMS
provider has not been selected or implemented for production, so this
repository is not ready to send live messages.
...
- Provider-neutral SMS boundary, with `dry-run` as the only configured adapter
...
| `SMS_PROVIDER`                         | `dry-run`        | Selected adapter; keep this value until provider work is complete |
...
or documentation. Provider-specific variables should be introduced only after
a provider decision and contract review.
...
Provider selection is deliberately open. [PROVIDER_AUDIT.md](PROVIDER_AUDIT.md)
records the minimum contract and operational questions that must be resolved
before an adapter or credentials are added. Documentation must be rechecked
against the selected provider's then-current official material before live
development begins.
...
The Blueprint keeps SMS in provider-neutral dry-run mode. `APP_BASE_URL`, admin
credentials, and the worker session secret still need to be supplied through
Render's secret environment settings. The web service generates its own
`SESSION_SECRET`; set a separate 32-or-more-character value on the worker. The
worker validates the setting but does not issue or verify web login sessions.
